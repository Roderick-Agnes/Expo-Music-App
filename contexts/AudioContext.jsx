...
const AudioContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchHotTracks = async (tracks) => {
      ...
      const response = await axios.post(`${LOCAL_SERVER_API_URL}/track-urls`, {
  ...
  useEffect(() => {
    const fetchPlaylistTracks = async () => {
      const { data } = await axios.get(
        `${LOCAL_SERVER_API_URL}/playlist-tracks/${playlistIsPlaying[0]?.id}`
      );
      ...
      setPlaylistTracksIsPlaying(response.data.data);
    };
