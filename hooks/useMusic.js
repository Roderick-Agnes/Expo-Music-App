...
import { LOCAL_SERVER_API_URL, SERVER_API_URL } from "./../configs/index";
import {
  setGenres,
  setPlaylists,
  setTopAlbums,
...
const fetchGenres = async () => {
  try {
    const genres = await axios.get(LOCAL_SERVER_API_URL + "/genres");
    return genres;
  } catch (error) {
...
const fetchPlaylists = async (playlistId) => {
  try {
    const playlist = await axios.get(LOCAL_SERVER_API_URL + "/playlist", {
      params: {
        playlistId,
      },
    });
...
const fetchTrack = async (trackId) => {
  try {
    const { data } = await axios.get(
      LOCAL_SERVER_API_URL + `/track/${trackId}`
    );
    return data;
  } catch (error) {
...
const fetchTopTracks = async () => {
  try {
    const { data } = await axios.get(LOCAL_SERVER_API_URL + "/top-tracks");
...
const fetchTopArtists = async () => {
  try {
    const { data } = await axios.get(LOCAL_SERVER_API_URL + "/top-artists");
...
const fetchTopAlbums = async () => {
  try {
    const { data } = await axios.get(LOCAL_SERVER_API_URL + "/top-albums");
...
const fetchFeaturedPlaylists = async (limit) => {
  try {
    const { data } = await axios.get(
      `${LOCAL_SERVER_API_URL}/featured-playlists?limit=${limit}`
    );
    return data;
  } catch (error) {
